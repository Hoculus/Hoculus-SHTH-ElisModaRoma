...
var animateCompleteTheLook = (function (node) {
  ...
  if (a$1(node, classes$D.imageLeft) || window.matchMedia(getMediaQuery("below-720")).matches) {
    delayItems.push(selectors$1A.media);
...
function giftCardRecipient (container) {
  ...
  var elements = {
    ...
    form: {
      ...
      sendOnInput: n$2(selectors.form.messageInput, formRecipientFormContainer),
  ...
  u$1(formRecipientFormContainer, "visually-hidden"); // Disable form inputs by default

  disableInputs(disableableInputs(), true);
  elements.form.offsetInput.value = new Date().getTimezoneOffset(); // Set up listeners for the display inputs
...
var Product = /*#__PURE__*/function () {
  function Product(node) {
    ...
    this.desktopMoreMedia = moreMedia(this.desktopMedia); // Handle Surface pickup
    ...
    this.siblingProducts = siblingProducts(this.container); // Gift card recipient
  ...
  _createClass(Product, [{
    ...
    value: function onFormSubmit(e) {
      ...
      cart.addItem(this.formElement).then(function (_ref4) {
        ...
        if (purchaseConfirmation) {
          ...
          r$1("quick-cart:updated"); // Need a delay to allow quick-cart to refresh
        ...
        if (error && error.message) {
          if (_typeof(error.message) === "object") {
            var sectionID = n$2(selectors$E.giftCardRecipientContainer, _this4.container).dataset.sectionId;
            Object.entries(error.message).forEach(function (_ref5) {
              ...
              var errorMessage = n$2("#".concat(errorMessageID), _this4.container);
              var errorInput = n$2("#display-gift-card-recipient-".concat(key, "--").concat(sectionID), _this4.container);
    ...
    value: function unload() {
      var _this$quantityInput, _this$mobileSwiper2, _this$stickyScroll, _this$mobileMoreMedia, _this$desktopMoreMedi, _this$featuredProduct, _this$variantAvailabi, _this$siblingProducts, _this$giftCardRecipie;
...
function PredictiveSearch(resultsContainer) {
  ...
  function getSearchResults(searchTerm) {
    ...
    fetch("".concat(window.theme.routes.predictive_search_url, "?q=").concat(encodeURIComponent(searchTerm), "&section_id=predictive-search")).then(function (response) {
      if (!response.ok) {
...
function Navigation(node, headerSection) {
  ...
  var header = n$2('[data-section-type="header"]', document.body);
...
register("quick-cart", {
  ...
  refreshQuickCart: function refreshQuickCart() {
    ...
    var url = "".concat(theme.routes.cart.base, "?section_id=").concat(this.id);
  ...
  scrollUpQuickCart: function scrollUpQuickCart() {
    ...
    if (!form) return; // delay the scroll up to make it seem more 'fluid'
